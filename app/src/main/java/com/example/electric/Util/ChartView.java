...
//https://blog.csdn.net/u014544193/article/details/54313257?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522168086259216800186551314%2522%252C%2522scm%2522%253A%252220140713.130102334.pc%255Fblog.%2522%257D&request_id=168086259216800186551314&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~blog~first_rank_ecpm_v1~rank_v31_ecpm-8-54313257-null-null.blog_rank_default&utm_term=android%20%E8%87%AA%E5%AE%9A%E4%B9%89%E6%8A%98%E7%BA%BF%E5%9B%BE&spm=1018.2226.3001.4450
...
public class ChartView extends View {
    ...
    //xy坐标轴文字大小
    private int xytextsize = spToPx(14);
    //折线图中折线的颜色
        ...
        if (changed) {
            ...
            for (int i = 0; i < yValue.size(); i++) {//求取y轴文本最大的宽度
                ...
                if (temp > textYWdith)
                    textYWdith = temp;
                //记录最高Y轴高度
                if(yValue.get(i) > maxY) maxY = yValue.get(i);
            }
            int dp2 = dpToPx(2);
            int dp3 = dpToPx(3);
...
//                    canvas.drawRoundRect(x - xValueRect.width() / 2 - dpToPx(2), yOri + xylinewidth + dpToPx(1), x + xValueRect.width() / 2 + dpToPx(3), yOri + xylinewidth + dpToPx(2) + xValueRect.height() + dpToPx(2), dpToPx(2), dpToPx(2), linePaint);
                } else {
                    canvas.drawText(text, 0, text.length(), x - rect.width() / 2, yOri + xylinewidth + dpToPx(2) + rect.height(), xyTextPaint);
                }
            }
